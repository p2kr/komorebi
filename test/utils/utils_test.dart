...
void main() {
  group('Utils - String formatting and helpers', () {
    test(
      'given name when getInitials called then returns correct uppercase initials',
      () {
        // Given & When & Then
    ...
    test(
      'given dateTime when getDateOnly called then returns formatted date or fallback',
      () {
        // Given
        final testDate = DateTime(2026, 7, 7, 12, 0, 0);
        final expectedFormat = DateFormat().add_yMd().format(
          testDate.toLocal(),
        );
  ...
  group('Utils - Widget helpers', () {
    testWidgets(
      'given SyncType when getSyncTypeIcon called then returns correct widget structure',
    ...
    testWidgets(
      'given Profile when getAvatar called then builds CircleAvatar with appropriate child or provider',
      (WidgetTester tester) async {
        // Given
