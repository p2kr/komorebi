...
void main() {
  ...
  group('CurrentProfileNotifier Tests', () {
    test('given empty DB when currentProfileProvider read then returns null', () async {
      // Given empty DB
    ...
    test('given profiles in DB without config when currentProfileProvider read then returns latest profile and sets LAST_USED_PROFILE config', () async {
      // Given
      ...
      // When
      final profile = await container.read(currentProfileProvider.future);
      final configVal = await db.configsDao.getConfig(Settings.LAST_USED_PROFILE.name);
    ...
    test('given LAST_USED_PROFILE config when currentProfileProvider read then returns configured profile', () async {
      // Given
      ...
      await db.configsDao.setConfig(Settings.LAST_USED_PROFILE.name, id1.toString());
    ...
    test('given invalid LAST_USED_PROFILE config when currentProfileProvider read then falls back to latest profile', () async {
      // Given
      final id1 = await db.profilesDao.insertProfile(
        ProfilesCompanion.insert(
      ...
      await db.configsDao.setConfig(Settings.LAST_USED_PROFILE.name, '9999'); // Non-existent ID
    ...
    test('given new profile when updateCurrentProfile called then updates state and saves config in DB', () async {
      // Given
      await db.profilesDao.insertProfile(
      ...
      // When
      await container.read(currentProfileProvider.notifier).updateCurrentProfile(newProfile);
  ...
  group('allProfilesProvider Tests', () {
    test('given multiple profiles in DB when allProfilesProvider read then streams all active profiles', () async {
      // Given
      await db.profilesDao.insertProfile(
